...
CREATE TABLE IF NOT EXISTS pkw (
    ...
    ncapRating   INT NOT NULL CHECK (rating >= 0 AND rating <= 5),
    motor         ENUM('VERBRENNER', 'ELEKTRO'),
...
) TABLESPACE pkwspace ROW_FORMAT=COMPACT;
ALTER TABLE pkw AUTO_INCREMENT=1000;

CREATE TABLE IF NOT EXISTS bauart (
    id          INT NOT NULL PRIMARY KEY AUTO_INCREMENT,
    model       VARCHAR(40) NOT NULL,
    variante    VARCHAR(40) NOT NULL,
    pkw_id      CHAR(36) UNIQUE NOT NULL references pkw(id)
) TABLESPACE pkwspace ROW_FORMAT=COMPACT;
ALTER TABLE bauart AUTO_INCREMENT=1000;
