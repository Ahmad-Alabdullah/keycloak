...
import { type PkwModel } from '../../src/pkw/rest/pkw-get.controller.js';
...
describe('GET /rest/:id', () => {
    ...
    beforeAll(async () => {
        ...
        client = axios.create({
            baseURL,
            httpsAgent,
            validateStatus: (status: any) => status < 500, // eslint-disable-line @typescript-eslint/no-magic-numbers
        });
    });
    ...
    test('Pkw zu vorhandener ID', async () => {
        ...
        // when
        const response: AxiosResponse<PkwModel> = await client.get(url);
    ...
    test('Kein Pkw zu nicht-vorhandener ID', async () => {
        // given
    ...
    test('Pkw zu vorhandener ID mit ETag', async () => {
        // given
